...
uint256 hashGenesisBlock("0xd0e9d5a9f2dc4aeb64e8055a4a6afdfc7d374593a24fb64f094c0d770ad5c5cf");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Angelcoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Angelcoin Signed Message:\n";
...
{
    // Angelcoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    else if (nHeight >= 500 && nHeight < 5500)
    {
        nSubsidy = 5000 * COIN;
    }

    // Subsidy is cut in half every 50000 blocks, which will occur approximately every 0.57 year (208.33 days)
    nSubsidy >>= (nHeight / 50000); // Angelcoin: 50k blocks in ~0.57 year (208.33 days)

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 8.4 * 24 * 60 * 60; // Angelcoin: 8.4 days
static const int64 nTargetSpacing = 6.0 * 60; // Angelcoin: 6 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Angelcoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Angelcoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // Angelcoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0xc264d7455c96216a85d6fba090efa64cc2b5915bd34f3983c3560089be22fbbf");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
		const char* pszTimestamp = "Wipe that look off your face, Mason. You have got so used to losing, you do"; //max 112 chars, otherwise error during start! All strings must contain only ASCII printable characters, must not contain quotes ["'] nor backslashes [\], must be no more than 256 bytes long
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 1000 * COIN;
		txNew.vout[0].scriptPubKey = CScript() << ParseHex("04C34E86D18978CDECC5CCD8249DC6FCA108EA59F6345B1363D0B07D7A400EEAE725F59CCAE1743D19D1B964D336776C2FD3072454F75CE8A043B026D2365A66A6") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1518588153;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 1139527614;

        if (fTestNet)
        {
            block.nTime    = 1518590713;
            block.nNonce   = 1137731000;
        }
        ...
        assert(block.hashMerkleRoot == uint256("0x1aa8896d189337472821f0a0fde254b578e491cbee8387666f3971a9e2ce694b"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xa0, 0xb0, 0xc0, 0xdb }; // Angelcoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("AngelcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("AngelcoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static AngelcoinMiner(CWallet *pwallet)
{
    printf("AngelcoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("Angelcoin-miner");
    ...
    try { loop {
        ...
        printf("Running AngelcoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("AngelcoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&AngelcoinMiner, pwallet));
}
